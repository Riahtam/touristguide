...
<form method="POST"  enctype="multipart/form-data" action="abtplace.php">
  ...
  {
		$sql9="select * from guesshouse limit $start_from,$num_per_page";
		//echo"<br>$no";
		$result=mysqli_query($conn,$sql9);
		if(mysqli_num_rows($result)>0)
				{
					while($row = mysqli_fetch_array($result))
					{
                       ...
                       // $district=$row['district'];
                        echo " <br><b style='margin-left:53%;font-size:28px'>$n </b><br>";
				...
				if(mysqli_num_rows($result)>0)
						{
							while($row = mysqli_fetch_array($result))
							{
                                ...
                                ?>
							      <a href='abtguess.php?iv= <?php echo $row['guessid'];?>'  style='color:black'>MORE</a>  
 ...
 echo"<center>";
  ...
  $pr_query="select * from guesshouse";
 ...
 if($page>1)
 {
	...
	echo "<a href='guess.php?page=" .($page-1) . " ' style='font-size:22px'> Previous </a> ";
...
for($i=1;$i<$total_page;$i++)
{
	...
	echo  "<a href='guess.php?page=" . $i . " ' style='font-size:22px'> $i </a>";


}
if($i>$page)
{
   echo " <a href='guess.php?page=" .($page+1) . " ' style='font-size:22px'> Next </a> ";
