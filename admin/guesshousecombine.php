...
<a href="dashboard.php" style="margin-left:82%">HOME</a>         &nbsp; &nbsp; <a href="newguesshouse.php">ADD NEW </a> <br>
<br>
...
if(isset($_POST['sub']))
{
	   ...
	   $name=$_POST['nam'];
	   $sql9="select * from guesshouse where name='$name' ";
		   //echo"<br>$no";
		...
		//if(mysqli_num_rows($result)>0)
          {
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					<td>  <a href="#=<?php echo $row["guessid"];?>"
	...
	header("location:guesshousecombine.php");
}
  ...
  else
  {
	...
	$sql9="select * from guesshouse   ORDER by guessid DESC   ";
	//echo"<br>$no";
 $result=mysqli_query($conn,$sql9);
 if(mysqli_num_rows($result)>0)
   {
	 echo"<center>";
					 echo "<h3><u>GUESS HOUSE DETAIL<br></u></h3>";
		 
	  echo "<table border=1 width=60%>
					  <tr>
					  <th>IMAGE</th>
						  <th> GUESS HOUSE  NAME</th>
						  <th> DISTRICT</th>
						  <th>LOCALITY</th>
						  <th>ABOUT</th>
						  <th colspan='2'>ACTION</th>
		 ...
		 while($row = mysqli_fetch_assoc($result))
		 {
				 echo "<tr>";?>
				   ...
				   echo "<td>".$row["about"]."</td>";?>
					...
					<td>  <a href="guesshousecombine.php?edit=<?php echo $row["guessid"];?> "  class="btn btn-info">EDIT </a>
				</td>
			
					<td>  <a href="guesshousecombine.php?delete=<?php echo $row["name"];?> " class="btn btn-danger">DELETE</a>
					</td>
...
{
  $na=$_GET['delete'];
  $sql1="delete from guesshouse where name='$na'";
...
if(isset($_GET['edit']))
{   $id=$_GET['edit'];
	$sql12="select   *  from guesshouse where guessid='$id' ";
...
<?php  if(isset($_POST['up']))
{
	...
	$a=$_POST["content"];
			...
			$sql4a="update guesshouse set name='$n',guessid='$id', district='$d',locaname='$l',longti='',latit='', about='$a', image='$img' where guessid='$id' ";
