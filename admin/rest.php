...
<form method="POST"  enctype="multipart/form-data" action="abtplace.php">
  ...
  {
		$sql9="select * from restau limit $start_from,$num_per_page";
		//echo"<br>$no";
		$result=mysqli_query($conn,$sql9);
		if(mysqli_num_rows($result)>0)
				{
				...
				if(mysqli_num_rows($result)>0)
						{
							while($row = mysqli_fetch_array($result))
							{
                                ...
                                ?>
							      <a href='morerest.php?iv= <?php echo $row['id'];?>'  style='color:black'>MORE</a>  
 ...
 if($page>1)
 {
	...
	echo "<a href='rest.php?page=" .($page-1) . " ' style='font-size:22px'> Previous </a> ";
...
for($i=1;$i<$total_page;$i++)
{
	...
	echo  "<a href='rest.php?page=" . $i . " ' style='font-size:22px'> $i </a>";


}
if($i>$page)
{
   echo " <a href='rest.php?page=" .($page+1) . " ' style='font-size:22px'> Next </a> ";
